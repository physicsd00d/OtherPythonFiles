...
customFacetFile   = 'LevelOne_CustomOutput_FalconCapeTraditional_TechCenterTimes_UnlimitedOnly'
customAirTopFile  = 'LevelOne_FromAirtop'
...
aircraftDictFacet,   sectorDictFacet    = readCustomFacetFile(resultsFolder + customFacetFile)
aircraftDictAirtop, sectorDictAirtop    = readCustomFacetFile(resultsFolder + customAirTopFile)
...
reroutedAcListFacet = []
for curAC in sorted(aircraftDictFacet.keys()):
    tempVec = np.array(aircraftDictFacet[curAC]['isRerouted'])
    if sum(tempVec == CURR_AVOIDANCE_RTE) > 5:
    # if CURR_AVOIDANCE_RTE in tempVec:
        reroutedAcListFacet.append(curAC)

reroutedAcListAirtop = []
for curAC in sorted(aircraftDictAirtop.keys()):
    tempVec = np.array(aircraftDictAirtop[curAC]['isRerouted'])
    # if sum(tempVec == CURR_AVOIDANCE_RTE) > 1:
    if CURR_AVOIDANCE_RTE in tempVec:
        reroutedAcListAirtop.append(curAC.split('_')[0])            #'''REMOVING _0 HERE!'''

# reroutedAcList = sorted(aircraftDictComparison.keys())


# Those lists have appended IDs, strip them, and set them
facetSet = set([elem.split('_')[0] for elem in reroutedAcListFacet])
airtopSet = set([elem.split('_')[0] for elem in reroutedAcListAirtop])

# airtop uses hashes to indicate multiple aircraft of the same callsign.  Remove those for set purposes
# airtopSet = set([elem.split('#')[0] for elem in airtopSet])
flightsInCommon = facetSet.intersection(airtopSet)

print "FACET  has {0} total flights".format(len(aircraftDictFacet))
print "Airtop has {0} total flights".format(len(aircraftDictAirtop))
print "FACET  has {0} rerouted flights".format(len(reroutedAcListFacet))
print "Airtop has {0} rerouted flights".format(len(reroutedAcListAirtop))
print "They share {0} flights in common".format(len(facetSet.intersection(airtopSet)))
print facetSet.intersection(airtopSet)
